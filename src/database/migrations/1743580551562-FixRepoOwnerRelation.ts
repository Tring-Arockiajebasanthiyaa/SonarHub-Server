...
export class FixRepoOwnerRelation1743580551562 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    const hasOldOwnerColumn = await queryRunner.hasColumn(
      "repositories",
      "owner",
    );
    ...
    await queryRunner.query(
      `ALTER TABLE "repositories" ADD COLUMN "owner_id" uuid`,
    );
    ...
    await queryRunner.query(
      `ALTER TABLE "repositories" ALTER COLUMN "owner_id" SET NOT NULL`,
    );

    await queryRunner.query(
      `ALTER TABLE "repositories" DROP CONSTRAINT IF EXISTS "FK_repositories_owner"`,
    );
    await queryRunner.query(`ALTER TABLE "repositories" DROP COLUMN "owner"`);

    await queryRunner.query(
      `ALTER TABLE "repositories" RENAME COLUMN "owner_id" TO "owner"`,
    );
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE "repositories" ADD COLUMN "owner_username" character varying`,
    );
    ...
    await queryRunner.query(
      `ALTER TABLE "repositories" RENAME COLUMN "owner_username" TO "owner"`,
    );
