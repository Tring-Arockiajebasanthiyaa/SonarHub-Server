...
export class UserActivityResolver {
  private readonly SONARQUBE_URL = process.env.SONARQUBE_API_URL || "http://localhost:9000";
  private readonly SONARQUBE_TOKEN = process.env.SONARQUBE_API_TOKEN || "";
  private readonly GITHUB_API_URL = process.env.GITHUB_API;
  ...
  async getUserActivity(
    ...
    try {
      ...
      if (!githubResponse.ok) {
        throw new Error(
          `GitHub API Error: ${githubResponse.status} - ${await githubResponse.text()}`,
        );
      }
      ...
      for (const repo of repos) {
        ...
        if (
          !earliestRepo ||
          new Date(repo.created_at) < new Date(earliestRepo.created_at)
        ) {
          earliestRepo = repo;
        }
        ...
        if (
          !latestUpdatedRepo ||
          new Date(repo.updated_at) > new Date(latestUpdatedRepo.updated_at)
        ) {
          latestUpdatedRepo = repo;
        }
        const { data: repos } = await axios.get(
          `${process.env.GITHUB_API}/user/repos`,
          {
        ...
        try {
          ...
          if (sonarIssuesResponse.ok) {
            ...
            console.warn(
              `Failed to fetch SonarQube issues for ${repo.name}: ${sonarIssuesResponse.status}`,
            );
          }
        } catch (sonarError) {
          console.warn(
            `Error fetching SonarQube data for ${repo.name}:`,
            sonarError,
          );
        }
      }
      ...
      const issuePercentage =
        totalRepositories > 0
          ? (overallSonarIssues / totalRepositories) * 100
          : 0;
      ...
      if (userActivity) {
        console.log(
          `[INFO] Updating existing user activity for ${githubUsername}`,
        );
        ...
        userActivity.topContributedRepo = latestUpdatedRepo
          ? latestUpdatedRepo.name
          : "";
        userActivity.earliestRepoCreatedAt = earliestRepo
          ? earliestRepo.created_at
          : null;
        userActivity.mostRecentlyUpdatedRepo = latestUpdatedRepo
          ? latestUpdatedRepo.updated_at
          : null;
        ...
        userActivity = userActivityRepository.create({
          ...
          mostRecentlyUpdatedRepo: latestUpdatedRepo
            ? latestUpdatedRepo.updated_at
            : null,
          lastActive: new Date(),
          commitHistory,
