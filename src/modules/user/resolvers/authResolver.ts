...
import { sendMail } from "../../../utils/emailServices";
...
export class AuthResolver {
  ...
  async checkAuth(
    ...
    const userContext = ctx.req.user as {
      u_id: string;
      email: string;
      username: string;
    };
  ...
  async forgotPassword(
    ...
    try {
      ...
      const token = jwt.sign({ u_id: user.u_id }, this.JWT_SECRET, {
        expiresIn: "15m",
      });
      return {
  ...
  async resetPassword(
    ...
    try {
      ...
      const user = await dataSource
        .getRepository(User)
        .findOne({ where: { u_id: payload.u_id } });
  ...
  async signIn(
    ...
    return jwt.sign({ u_id: user.u_id }, jwtSecret, {
      expiresIn: "1d",
    });
  ...
  async setPassword(
    ...
    try {
      ...
      await sendMail(
        user.email,
        "Password Reset Confirmation",
        `Hi ${user.username || "there"},\n\nYour password has been successfully set. If this wasn't you, please contact support immediately.\n\nRegards,\nTeam`,
      );
