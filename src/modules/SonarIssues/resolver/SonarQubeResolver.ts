...
const ANALYSIS_IN_PROGRESS = "In Progress";
const ERROR_REPO_NOT_FOUND = "Repo not found";
...
export class SonarQubeResolver {
  ...
  async getProjectAnalysis(
    ...
    try {
      const projectKey = `${githubUsername}_${repoName.trim()}`.replace(
        /[^a-zA-Z0-9_-]/g,
        "_",
      );
      ...
      if (!user || !user.githubAccessToken) {
        throw new Error(
          `GitHub access token not found for user ${githubUsername}`,
        );
      }
  ...
  async triggerBranchAnalysisIfPROpen(
    ...
    try {
      ...
      if (!branchResponse.ok) {
        throw new Error(
          `Branch ${branchName} not found in repository ${repoName}`,
        );
      }
      ...
      const projectKey = `${githubUsername}_${repoName}_${branchName}`.replace(
        /[^a-zA-Z0-9_-]/g,
        "_",
      );
      ...
      try {
        ...
        const repo = await this.repoDetail.findOne({
    ...
    try {
      ...
      try {
        ...
        if (!repoResponse.ok) {
          ...
          throw new Error(
            `GitHub API error: ${repoResponse.status} - ${errorData.message}`,
          );
        }
  ...
  private async analyzeRepository(user: User, repo: any) {
    const projectKey = `${user.username}_${repo.name}`.replace(
      /[^a-zA-Z0-9_-]/g,
      "_",
    );
    const authHeader = `Basic ${Buffer.from(`${SONARQUBE_API_TOKEN}:`).toString("base64")}`;
    ...
    try {
      ...
      throw new Error(
        `Failed to get lines of code: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
    ...
    if (!project) {
      ...
      project.result =ANALYSIS_IN_PROGRESS;
  ...
  private async configureSonarQubeProject(
    ...
    try {
      ...
      for (const prop of propertiesToSet) {
        ...
        if (!response.ok) {
          console.error(
            `Failed to set property ${prop.key}:`,
            await response.text(),
          );
        }
      }
  ...
  private async configureSonarQubeWebhook(
    project: Project,
    authHeader: string,
  ) {
    try {
      ...
      if (listResponse.ok) {
        ...
        const existingWebhook = webhooks.webhooks.find(
  ...
  private async triggerSonarQubeAnalysis(
    ...
    try {
      ...
      if (!user?.githubAccessToken) {
        throw new Error(
          `GitHub access token not found for user ${project.username}`,
        );
      }
      ...
      if (!branchesResponse.ok) {
        throw new Error(
          `Failed to fetch branches: ${await branchesResponse.text()}`,
        );
      }
      ...
      for (const branch of branches) {
        ...
        const sonarProjectKey =
          `${user.username}_${project.title}_${branchName}`.replace(
            /[^a-zA-Z0-9_-]/g,
            "_",
          );
        ...
        try {
          ...
          await this.waitForProjectAnalysis(
            sonarProjectKey,
            authHeader,
            branchName,
          );
  ...
  private async waitForProjectAnalysis(
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      try {
        ...
        console.log(
          `Waiting for analysis to complete (Attempt ${attempt}/${maxAttempts})...`,
        );
  ...
  private async cloneRepository(
    ...
    const repoPath = path.join(
      normalizedTempDir,
      `repo-${branch.replace(/[^a-zA-Z0-9_-]/g, "_")}`,
    );
    const { execFileSync } = require("child_process");

    execFileSync(
      "git",
      ["clone", "-b", branch, "--single-branch", githubUrl, repoPath],
      { stdio: "inherit" },
    );
  ...
  private async runSonarScanner(
    ...
    try {
      ...
      const propertiesPath = path
        .join(normalizedRepoPath, "sonar-project.properties")
        .replace(/\\/g, "/");
      ...
      console.error(
        `SonarScanner failed for ${branchName}:`,
        error.message || error,
      );
      throw new Error(
        `SonarScanner failed: ${error.message || "Unknown error"}`,
      );
    }
  }
  ...
  private createSonarPropertiesFile(
    ...
    const workingDir = path
      .join(normalizedRepoPath, ".scannerwork")
      .replace(/\\/g, "/");
  ...
  private async updateProjectSettings(
    ...
    for (const setting of settings) {
      try {
        ...
        if (!response.ok) {
          console.warn(
            `Failed to set setting ${setting.key}:`,
            await response.text(),
          );
        }
      } catch (error) {
  ...
  private async getRepositoryLinesOfCode(
    ...
    try {
      ...
      const languageEntities = await dataSource
        .getRepository(LanguageBytesPerLineEntity)
        .find();
  ...
  public async handleWebhookEvent(
    ...
    try {
      ...
      console.log(SonarIssue,"Sonarissues")
      console.log(project);
      if (!project) {
      ...
      project.analysisDuration = Math.floor(
        (project.analysisEndTime.getTime() -
          project.analysisStartTime.getTime()) /
          1000,
      );
  ...
  private async storeAnalysisResults(
    ...
    try {
      const projectKey =
        `${project.username}_${project.title}_${branchName}`.replace(
          /[^a-zA-Z0-9_-]/g,
          "_",
        );
      ...
      const measures = await this.getBranchMeasures(
        projectKey,
        sourceBranch,
        authHeader,
      );

      const issues = await this.getBranchIssues(
        projectKey,
        sourceBranch,
        authHeader,
      );

      const metrics = await this.getCodeMetrics(
        projectKey,
        sourceBranch,
        authHeader,
      );
      ...
      console.error(
        `Failed to store analysis results for branch ${branchName}:`,
        error,
      );
      throw error;
    }
  ...
  private async getBranchMeasures(
    projectKey: string,
    branchName: string,
    authHeader: string,
  ): Promise<any> {
    ...
    const url =
      `${SONARQUBE_API_URL}/api/measures/component?component=${encodeURIComponent(projectKey)}` +
          ...
          switch (measure.metric) {
            ...
            case "ncloc_language_distribution":
              ...
              if (langValue.includes(";")) {
                const langDist: Record<string, number> = langValue
                  .split(";")
                  .reduce(
                ...
                const topLang = Object.entries(langDist).sort(
                  (a, b) => b[1] - a[1],
                )[0]?.[0];
        ...
        for (const measure of measures.component.measures) {
          switch (measure.metric) {
            ...
            case "duplicated_lines_density":
              codeMetrics.duplicatedLines = Math.round(
                measure?.duplicatedLines || 0,
              );
              break;
            case "ncloc":
              codeMetrics.linesOfCode = parseInt(measure.value);
    ...
    try {
      ...
      measures.forEach((measure: any) => {
        ...
        if (measure.metric === "ncloc_language_distribution") {
          ...
          totalLines =
            Object.values(languageDistribution).reduce(
              (sum, val) => sum + val,
              0,
            ) || totalLines;
        }
  ...
  private parseLanguageDistribution(
    distribution: string,
